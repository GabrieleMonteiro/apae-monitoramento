...
router.get('/', async (req, res) => {
  try {
    const [linhas] = await db.query(`
      SELECT f.tb03_id_feedback, f.tb03_comentario, f.tb03_causa,
        f.tb03_hora, f.tb03_data, f.tb03_id_professor,
        p.tb04_nome AS nome_professor, f.tb03_id_leitura,
        l.tb02_temperatura, l.tb02_nivel_som
...
router.post('/', async (req, res) => {
  ...
  if (!tb03_id_professor || !tb03_id_leitura) {
    return res.status(400).json({ erro: 'id_professor e id_leitura sao obrigatorios' });
  }
  try {
